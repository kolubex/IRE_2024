...
class processor(object):
    def __init__(self, lang, code=None, remove_comments=False):
        ...
        if code is None:
            ...
            self.update(code)
        self.vnames = ["data", "content", "mean", "std", "file", "input", "list", "result", "value", "item"]
        self.fnames = ["process", "calculate", "get", "set", "fetch", "read", "write", "execute", "transform"]
    ...
    def process(self, ratio=0.85, indent=True, add_dead_code=True, cut_ratio=0.0):
        ...
        num_vnames_to_sample = min(int(len(vnames) * ratio), len(vnames))
        num_cands_to_sample = min(len(vnames) + 3, len(self.vnames))
        vnames = random.sample(vnames, num_vnames_to_sample)
        cands = random.sample(self.vnames, num_cands_to_sample)
        dead_vars = cands[-3:]
        if add_dead_code:
